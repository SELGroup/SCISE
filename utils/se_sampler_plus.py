...
class NeighborSampler(torch.utils.data.DataLoader):
    ...
    def get_batch(self, random_nodes):
        ...
        # 2. 向量化计算社区大小 (修复 TypeError)
        # 使用 bincount 替代 len(c) 循环，避开 comm_list 结构问题
        ...
        rw_seed_comms = node_comm_ids[large_mask].view(-1, 1) # [num_rw_seeds, 1]
        
        # --- 处理大社区 (保持原逻辑：同社区约束 + 频率过滤) ---
        if rw_seeds.numel() > 0:
        ...
        # stage two: 形成batch子图并赋边权（用于损失计算）
    ...
    def get_batch_wo_BExtend(self, random_nodes):
        device = random_nodes.device
        batch_set = set(random_nodes.tolist())
        '''
        # 1. 获取起始节点社区 ID
    ...
    def get_batch_wo_StructCL(self, random_nodes):
        device = random_nodes.device
        batch_set = set(random_nodes.tolist())
        ...
        adj_batch, _ = self.adj.saint_subgraph(batch)

        # 2. 去除自环 (Self-loops)
        # 按照你之前的逻辑，需要将对角线置 0
        adj_batch = adj_batch.remove_diag()
        '''
        # stage two: 形成batch子图并赋边权（用于损失计算）
        batch_size = batch.shape[0]
        batch_repeat = batch.repeat(self.wt)
    ...
    def sample(self, batch):
        ...
        adj_batch = None
        if self.is_train:
            batch_ex, adj_batch = self.get_batch(batch)
            #batch, adj_batch = self.get_batch_wo_BExtend(batch)
            #batch, adj_batch = self.get_batch_wo_StructCL(batch)
        else:
            batch_ex = batch
        batch_size: int = len(batch_ex)
        ...
        return out, adj_batch, batch_ex, batch
