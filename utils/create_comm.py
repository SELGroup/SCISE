...
def creat_comm(edge_index, N, save_path, args, draw_fig=False, cal_se=True):
    ...
    checkpoint = {
        'comm': comm.cpu(), #每个node对应社区
        'comms': {i: v.cpu() for i, v in comms.items()}, #每个社区包含的节点
    }
    ...
    if draw_fig:
        ...
        for size in sorted_sizes:
            ...
            #print(f"节点数为 {size} 的社区共有: {count} 个")
    ...
    #S1 = torch.randint(0, totol_comm, (N,)).to(args.device)
    #_,SErand = cal_se_hard(deg.to(args.device), S1, edge_index) 
    #print(f'random se={SErand:.4f}')
