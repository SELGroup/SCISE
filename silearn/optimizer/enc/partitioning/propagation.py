...
class OperatorPropagation(Operator):
    ...
    def perform(self,
        ...
        while operated_cnt > ter or not merge_all:
            #print(f"operated_cnt:{operated_cnt}, ter:{ter}, merge_all:{merge_all}")
            ...
            #print(f"merge shape:{merge.shape}, {torch.any(merge)}") 
            ...
            # print(operated_cnt)
            if operated_cnt == 0:
                #print('continue at operated_cnt=0')
                continue
            ...
            #trans = torch.arange(current_num_vertices+operated_cnt, device=self.enc.graph.device)
