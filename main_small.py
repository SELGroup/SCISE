...
from model_small import Model, Encoder
...
def train(args, ts):
    ...
    encoder = Encoder(num_features, hidden_channels=hidden, base_model=GCNConv, dropout=args.dropout, ns=args.ns).to(device)
...
if __name__ == '__main__':
    ...
    args.batchsize = 2048
    #path = f'./result/{args.dataset}/noise/neighbor_noise_SCISE.txt'
    path = f'./result/{args.dataset}/0_SCISE.txt'
    #args.commNum = 1
    for noise in [0.20]:
        #args.neighbor_noise_rate = noise
