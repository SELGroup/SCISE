...
from utils.util import setup_seed, get_mask, get_mask1, clustering, get_graph_stats
...
def train(args, ts):
    ...
    print(f"Loading {args.dataset} is over, num_nodes: {N: d}, num_edges: {E: d}, degree: {(E / N) :.5f}, "
          f"num_feats: {num_features: d}, time costs: {time.time()-ts: .3f}\n")
    if log_file is not None:
    ...
    N, E, num_features = x.shape[0], edge_index.shape[-1], x.shape[-1]
    print(f"processing {args.dataset} is over, num_nodes: {N: d}, num_edges: {E: d}, degree: {(E / N) :.5f}, "
          f"num_feats: {num_features: d}, time costs: {time.time()-ts: .3f}\n")
    if log_file is not None:
        with open(log_file, 'a') as file_log:
    ...
    if args.dataset == 'ogbn-papers100M':
        for m in [1]:
            for p in [1,0.9,0.7,0.5,0.3,0.1]:
                args.p = p
                args.commNum = m
    ...
    #if (not dataset[0].is_undirected()) and args.direct == 1:
    #    comm_path = f"./comms/{args.dataset}/direct/division_comm{args.commNum}_p{args.p}.pt"
    ...
    if not os.path.exists(comm_path):
        ...
        print(f"SE显存占用峰值: {peak_memory:.5f} GB, 分配峰值： {reserved_memory:.5f} GB, 内存占用峰值: {peak / (1024 ** 2):.5f} MB")
        if log_file is not None:
            with open(log_file, 'a') as file_log:
                file_log.write(f"SE显存占用峰值: {peak_memory:.5f} GB, 分配峰值： {reserved_memory:.5f} GB, 内存占用峰值: {peak / (1024 ** 2):.5f} MB\n")
    else:
        comm_data = torch.load(comm_path)
    comm = comm_data['comm'].long().clone()
    comms = comm_data['comms']
    print(f"Loaded {len(comms)} communities from {comm_path}.")
    if log_file is not None:
        with open(log_file, 'a') as file_log:
    ...
    if hasattr(args, 'comm_noise_rate') and args.comm_noise_rate != 0:
        ...
        print(f"--- Added {num_noise_nodes} noise comms (rate: {args.comm_noise_rate}) ---")
        if log_file is not None:
            with open(log_file, 'a') as file_log:
    ...
    encoder = Encoder(num_features, hidden_channels=hidden, base_model=SAGEConv, dropout=args.dropout, ns=args.ns).to(device)
    ...
    dataset2n_clusters = {'ogbn-arxiv': 40, 'Reddit': 41,
                          ...
                          'Pubmed':3, 'Computers': 10}
    n_clusters = dataset2n_clusters[args.dataset]
    #x = x.to(device)

    ari_max = [0, 0, 0, 0, 0, 0] #nmi, ari, acc, f1, all. epoch
    all_max = [0, 0, 0, 0, 0, 0]
    ...
    for epoch in range(1, args.epochs):
        ...
        for (batch_size, n_id, adjs), adj_batch, batch_ex, batch in train_loader:
            ...
            #print(out.shape, adj_mask.sizes())
            ...
            if args.dataset in ['Pubmed','Computers']:
                verbose = 1
            else:
                verbose = 10
        ...
        #gc.collect()
        #torch.cuda.empty_cache()
        if args.dataset in ['Pubmed','ogbn-products','ogbn-papers100M']:
            epoch_verbose = 1
            epoch_start = 1
        ...
        if (epoch >= epoch_start and epoch % epoch_verbose == 0) or (epoch == 1):
            ...
            outline =f'Epoch {epoch:02d}, loss: {total_loss:.3f}, nmi: {nmi:.5f}, ari: {ari:.5f}, acc: {acc:.5f}, f1_macro: {f1_macro:.5f}, all_metric: {all_metric:.5f}'
            
            # early stop
            improved = False
            if ari > ari_max[1]:
                ari_max = [nmi, ari, acc, f1_macro, all_metric, epoch]
                improved = True
                outline = outline + f', new max ARI !!!'
            ...
            if (step > args.step and epoch > 20) or (time.time()-ts)/60/60 >= 10:
                outline = "*********************** early stop ***********************"
                with open(save_file, 'a') as file1:
                    file1.write(f'{outline}\n')
                print(outline)
    ...
    outline = f'best perform ARI at epoch{ari_max[5]}, nmi: {ari_max[0]:.5f}, ari: {ari_max[1]:.5f}, acc: {ari_max[2]:.5f}, f1_macro: {ari_max[3]:.5f}, all_metric: {ari_max[4]:.5f}'
    with open(save_file, 'a') as file1:
        file1.write(f'{outline}\n')
    print(outline)
    if log_file is not None:
...
def draw_tsne(args):
    ...
    z_path = f'./checkpoints/{args.dataset}_embed_DeSE.pt'
    z = torch.load(z_path)
    ...
    '''
    fig, ax = plt.subplots(figsize=(10,10))
    scatter = plt.scatter(X_tsne[:, 0], X_tsne[:, 1], c=y, cmap=custom_cmap, s=50)
    plt.axis('off')
    plt.tight_layout()
    save_path = f"/workspace/UnLC-BiSE/draw/tsne_{args.dataset}_DeSE"
...
if __name__ == '__main__':
    ...
    if args.dataset == 'Pubmed':
        ...
        args.batchsize = 4096
        args.size = '15,15' #'6,6'
        args.hidden_channels = '1024,128' #'512,128'
        args.tau = 0.7 #0.9
        args.ns = 0.2 #0.2
        args.lr = 0.005
        ...
        args.batchsize = 4096
        args.size = '10,10,10'
        args.hidden_channels = '1024,512,256'
        args.tau = 0.9
        args.ns = 0.2
        args.lr = 0.005

        args.kmeans_device = 'cuda'
        args.kmeans_batch = 30000
    ...
    for noise in [0]:
        ...
        for i in range(1):
            ...
            ari_max, all_max, args = train(args, ts)
            with open(path, 'a') as file:
                if args.dataset == 'Pubmed':
                    file.write(f'\n{"-"*25} test {i+1} {"-"*25}\nRun at: {timestamp}, total time cost = {(time.time()-ts)/60:.3f} min\n')
                else:
                    file.write(f'\n{"-"*25} test {i+1} {"-"*25}\nRun at: {timestamp}, total time cost = {(time.time()-ts)/60/60:.3f} h\n')
                file.write(f'Args: {args}\n')
                result_line = f'        ARI MAX Result -> nmi: {ari_max[0]:.5f}, ari: {ari_max[1]:.5f}, acc: {ari_max[2]:.5f}, f1_macro: {ari_max[3]:.5f}, all: {ari_max[4]:.5f}\n'
