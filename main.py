...
from utils.util import setup_seed, get_mask, get_mask1, clustering
...
def train(args, ts):
    ...
    for epoch in range(1, args.epochs):
        ...
        if (epoch >= epoch_start and epoch % epoch_verbose == 0) or (epoch == 1):
            ...
            if (step > args.step and epoch > 20) or (time.time()-ts)/60/60 >= 4:
                outline = "*********************** early stop ***********************"
                with open(save_file, 'a') as file1:
                    file1.write(f'{outline}\n')
                print(outline)
...
if __name__ == '__main__':
    ...
    for i in range(3):
        ...
        with open(path, 'a') as file:
            file.write(f'\n{"-"*25} test {i+1} {"-"*25}\nRun at: {timestamp}, total time cost = {(time.time()-ts)/60/60:.2f} h\n')
